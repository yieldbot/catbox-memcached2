...
exports = module.exports = function(options) {
  ...
  if(serverEvents) {
    // Event stream
    var EventStream = function EventStream() { EventEmitter.call(this); };
    util.inherits(EventStream, EventEmitter);
    eventStream = new EventStream();
    eventStream.on('error', function() { }); // prevent unhandled error event
  }
  ...
  var start = function start(callback) {
    ...
    // Set events
    if(serverEvents) {
      // Function for emitting multiple events
      var eventEmit = function eventEmit(events, details) {
        for(var i = 0, len = events.length; i < len; i++) {
          eventStream.emit(events[i], details);
        }
      };

      memcached.on('issue',        function(details) { eventEmit(['issue', 'error',   'memcached'], details); });
      memcached.on('failure',      function(details) { eventEmit(['failure', 'error', 'memcached'], details); });
      memcached.on('reconnecting', function(details) { eventEmit(['reconnecting',     'memcached'], details); });
      memcached.on('reconnected',  function(details) { eventEmit(['reconnected',      'memcached'], details); });
      memcached.on('reconnect',    function(details) { eventEmit(['reconnect',        'memcached'], details); });
      memcached.on('remove',       function(details) { eventEmit(['remove', 'error',  'memcached'], details); });
    }

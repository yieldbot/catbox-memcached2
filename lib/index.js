/*
 * catbox-memcached2
 * Copyright (c) 2015 Yieldbot, Inc. (http://github.com/yieldbot/catbox-memcached2)
...
module.exports = function(options) {
  ...
  var get = function get(key, callback) {
    ...
    cacheClient.get(keyF, function(err, result) {
      if(err) {
        if(bypassCacheOnConnError && isConnError(err)) {
  ...
  var isConnError = function isConnError(err) {
    return (err && (err.code === 'ECONNREFUSED' || (typeof err.message === 'string' && err.message.indexOf('not available') !== -1)));
  };
